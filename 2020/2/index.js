...
const getValidPasswordsByMinMaxCount = () =>
  passwordSettings.reduce((acc, { min, max, letter, password }) => {
    const count = password.split('').filter((char) => char === letter).length;
    return (count >= min && count <= max) ? acc += 1 : acc;
  }, 0);

const getValidPasswordsByIndexCount = () =>
  passwordSettings.reduce((acc, { min: firstIndex, max: secondIndex, letter, password }) => {

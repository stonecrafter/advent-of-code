...
const { mapByAllergen, listByFood, mapByIngredient } = inputList.reduce(
  (acc, food, foodItemIndex) => {
...
const getAllergenCount = () => {
  ...
  const safeItems = Object.keys(mapByIngredient).reduce(
    (acc, item) => (unsafeItems.includes(item) ? acc : [...acc, item]), [],
  );
...
const getAllergenList = () => {
  ...
  const potentialAllergens = Object.keys(mapByAllergen).reduce((acc, key) => {
    const items = Array.from(new Set(mapByAllergen[key].filter(
      (item) => unsafeItems.includes(item),
    )));
    return {
      ...acc,
      [key]: items,
    ...
    unsafeItems.forEach((unsafeItem) => {
      listByFood.forEach((food) => {
        if (food.allergens.includes(allergen) && !food.items.includes(unsafeItem)) {
          ...
          potentialAllergens[allergen] = potentialAllergens[allergen].filter(
            (i) => i !== unsafeItem,
          );
        }
      });
      ...
      if (potentialItems.length === 1) {
        Object.keys(potentialAllergens).forEach((otherAllergen) => {
          if (otherAllergen !== allergen) {
            potentialAllergens[otherAllergen] = potentialAllergens[otherAllergen].filter(
              (i) => i !== potentialItems[0],
            );
          }
        });
      }

...
const bagsMap = bagsList.reduce((bagsAcc, bag) => {
  ...
  // muted crimson bags contain 1 clear violet bag, 5 dark coral bags,
  // 1 pale salmon bag, 3 light red bags.
  // ['muted crimson bags', '1 clear violet bag, 5 dark coral bags,
  // 1 pale salmon bag, 3 light red bags.']
...
const getTotalChildBags = (targetBag) => {
  ...
  return Object.keys(childBags).reduce(
    (acc, bagId) => acc + childBags[bagId] + childBags[bagId] * getTotalChildBags(bagId), 0,
  );
};

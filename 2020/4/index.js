...
const passportData = passportList.map((passport) => passport.split(/\s/).reduce((acc, dataField) => {
...
const getValidFieldsPassportsCount = () => passportData.reduce(
  (acc, passport) => (REQUIRED_FIELDS.every((field) => !!passport[field]) ? acc + 1 : acc), 0,
);

// Helper functions for validating the input of each field
const isValidNumRange = (year, min, max) => year && year >= min && year <= max;
...
const isValidHairColour = (hairColour) => hairColour && hairColour.match(/^#[0-9a-f]{6}$/);
...
const isValidPassportId = (id) => id && id.match(/^\d{9}$/);

// Put it all together
const getValidInputsPassportsCount = () => passportData.reduce((acc, passport) => {
